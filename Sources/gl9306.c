...
#define b_updata_uret uart2_flag // 光流数据更新标志
...
// uint8_t b_updata_uret;//光流数据更新标志
uint8_t pos_flag;
...
#define KKP 0.1f // 0.28 0.34//抗风试着增大这个系数！
#define KP 12.0f // 7.2
#define KD 1.0f  // 0.7
...
void OPTICAL2Read(void)
{
  ...
  // static int16_t angle_x_old,angle_y_old;//使用角度补偿光流的值
  if (b_updata_uret) // if 收到了新的光流数据包
  {
    b_updata_uret = 0;
    ...
    flow_decode();
    if ((abs(Remote.roll - 1500) <= 30) && (abs(Remote.pitch - 1500) <= 30)) // 左摇杆在中心位置时，使能光流
    {
      ...
      sum_flow_y = LIMIT(sum_flow_y, -50.0f, 50.0f); // 抗风增加此项   25.0f
      ...
      //      vy = constrainf(speed_y,-14.0f,14.0f);// + (angle_y_old - EulerPitch)*0.2f*scale;//利用欧拉角变换补偿光流输出值
      //      vx = constrainf(speed_x,-14.0f,14.0f);// - (angle_x_old -  EulerRoll)*0.2f*scale;//方向!?
...
static float PID_calculate(float T, float in_ff, float expect, float feedback, _PID_arg_st* pid_arg, _PID_val_st* pid_val, float inte_lim)
{
  ...
  if ((int16_t)(100 * pid_arg->inc_hz) != 0)
