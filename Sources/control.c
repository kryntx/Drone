...
PidObject* (pPidObject[]) = { &pidRateX, &pidRateY, &pidRateZ, &pidRoll, &pidPitch, &pidYaw, &pidHeightRate, &pidHeightHigh };
...
void FlightPidControl(float dt)
{
  ...
  switch (status)
  {
    ...
    case READY_11:            // 准备进入控制
      ...
      status = READY_12;
      break;
    case READY_12: // 等待油门拉起才进入 PID 控制，防止地面积分
      if (Remote.thr > 1100)
      {
        status = PROCESS_31;
      }
