...
uint8_t TX_ADDRESS[TX_ADR_WIDTH] = {0x11, 0x22, 0x33, 0x98, 0x67}; // 本地地址
uint8_t RX_ADDRESS[RX_ADR_WIDTH] = {0x11, 0x22, 0x33, 0x98, 0x67}; // 接收地址
...
uint8_t NRF24L01_RxPacket(uint8_t *rxbuf)
{
  ...
  if (sta & RX_OK)                                      // 接收到数据
  {
    ...
    if (len == 0 || len > TX_PLOAD_WIDTH)
    {
      NRF24L01_Write_Reg(FLUSH_RX, 0xff);                     // 清除RX FIFO寄存器
      NRF24L01_Write_Reg(NRF24L01_WRITE_REG + STATUS, RX_OK); // 清除无效长度中断标志
      return 0;
    }
...
void RX_Mode(void)
{
  ...
  NRF24L01_Write_Reg(NRF24L01_WRITE_REG + SETUP_AW, 0x03);                                 // 5字节地址宽度
  ...
  NRF24L01_Write_Reg(NRF24L01_WRITE_REG + RF_CH, 111);                                      // 设置RF通信频率
...
void TX_Mode(void)
{
  ...
  NRF24L01_Write_Reg(NRF24L01_WRITE_REG + RF_CH, 111);       // 设置RF频率
