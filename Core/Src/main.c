...
volatile uint8_t nrf2401_tx_flag = 1; 
...
uint8_t mpu_cnt = 0, mpu_flag = 0;
uint8_t volt_cnt = 0;

extern uint8_t RX_ADDRESS[RX_ADR_WIDTH];
extern _st_Mpu MPU6050;
extern uint8_t mpu_cal_flag; // MPU校准标志位，1表示需要校准，0表示校准完成

uint32_t low_power_tick = 0;
...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef* huart, uint16_t Size)
{
  if (huart->Instance == USART1)
  {
    if (huart->RxEventType == HAL_UART_RXEVENT_IDLE)
    {
      if (Size > 0)
      {
        low_power_tick = HAL_GetTick();
        memcpy(uart1_rx_temp, uart1_rx_buff, Size);
        uart1_rx_len = Size;
        HAL_UARTEx_ReceiveToIdle_DMA(&huart1, (uint8_t*)uart1_rx_buff, sizeof(uart1_rx_buff));
      }
      else
      {
        HAL_UARTEx_ReceiveToIdle_DMA(&huart1, (uint8_t*)uart1_rx_buff, sizeof(uart1_rx_buff));
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
  ...
  if (GPIO_Pin == NRF24L01_IRQ_Pin)
  {
    ...
    {
      RC_Analy();
      NRF24L01_Write_Buf(W_ACK_PLOAD0, nrf2401_txbuf, txbuf_pos);
      nrf2401_tx_flag = 1;
    }
    low_power_tick = HAL_GetTick();
  }
...
{
  ALL_flag.unlock = 0;
  HAL_NVIC_DisableIRQ(NRF24L01_IRQ_EXTI_IRQn);
  ...
  IIC_Write_One_Byte(MPU6050_ADDRESS, PWR_MGMT_1, 0x40);
  User_Data_Save_All();
...
int main(void)
{
  ...
  RX_Mode(); // 配置为接收模式
  HAL_NVIC_EnableIRQ(NRF24L01_IRQ_EXTI_IRQn); // 等NRL24L01初始化完后再开中断

  User_Data_Load_All(); // 读取Flash用户数据
  ...
  if (ALL_flag.paired == 0) // 如果未配对，则在此循环反复执行配对模式
  {
    LED.status = PAIRING;
    // PilotLED();
  }
  ...
  {
    LED.status = AllFlashLight; // 配对成功后，设置LED状态为全闪烁
  }

	if (RX_ADDRESS[0] != 0xFF)
	{
		RX_Mode();
	}
  
  send_char_array(&huart1, RX_ADDRESS, RX_ADR_WIDTH); // 配对成功后，发送接收地址到上位机

  User_Data_Save_All(); // 配对成功后，保存用户数据到Flash

  low_power_tick = HAL_GetTick();
  ALL_flag.sleep_timeout_s = 60;
  ...
  while (1)
  {
    ...
    if (mpu_cal_flag == 1)
    {
      ...
      HAL_TIM_Base_Stop_IT(&htim1);
      MpuGetOffset();
      HAL_NVIC_EnableIRQ(NRF24L01_IRQ_EXTI_IRQn);
      HAL_TIM_Base_Start_IT(&htim1);
			printf("cal\n");
      mpu_cal_flag = 0;
    }
    ...
    {
      SPL06_flag = 0;
      // printf("T:%3.2f P:%6.2f H:%6dcm\r\n", temperature, presure, baro_height);
    }
    if (uart1_rx_len > 0 && uart1_rx_len <= 32)
    {
      uart1_rx_len = 0;
    }
    if (uart2_flag)
    ...
    if ((HAL_GetTick() - low_power_tick) > (ALL_flag.sleep_timeout_s * 1000))
    {
      LowPowerMode();
    }
    /* USER CODE END WHILE */
